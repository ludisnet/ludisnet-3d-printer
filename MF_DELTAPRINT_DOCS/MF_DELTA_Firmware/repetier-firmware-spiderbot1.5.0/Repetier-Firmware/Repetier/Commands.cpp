...
void Commands::executeGCode(GCode *com)
{
    ...
    if(com->hasG())
    {
        ...
        case 29: // 3 points, build average
        {
            ...
            float zProbe2 = 0;
            float zProbe3 = 0;
            float zPrinterHeight = 0;
            float zMeasuredPrinterHeight = 0;
            float securityDistance = 20;
            float levelTolerance = 0.06;
            //Com::printFLN(Com::tEPRZMaxLength);
            ///Com::printFLN(Com::tZProbe,Printer::zLength);
            ///Com::printFLN(Com::tZProbe,Printer::zLength);
            Printer::moveTo(EEPROM::zProbeX1(),EEPROM::zProbeY1(),securityDistance,IGNORE_COORDINATE,EEPROM::zProbeXYSpeed());
            sum = Printer::runZProbe(true,true);
            zProbe1 = sum;
            ///Com::printFLN(Com::tZProbe,zProbe1);
            ///Com::printFLN(Com::tZProbe,Printer::zLength);
            Com::printFLN(Com::tZProbe,Printer::zLength - securityDistance + zProbe1);
            if(sum<0) break;
            uint8_t home_all_axis = (com->hasNoXYZ());
            Printer::moveTo(EEPROM::zProbeX2(),EEPROM::zProbeY2(),securityDistance,IGNORE_COORDINATE,EEPROM::zProbeXYSpeed());
            last = Printer::runZProbe(true,true);
            if(last<0) break;
            sum+= last;
            zProbe2 = last;
            ///Com::printFLN(Com::tZProbe,zProbe2);
            ///Com::printFLN(Com::tZProbe,Printer::zLength);
            Com::printFLN(Com::tZProbe,Printer::zLength - securityDistance + zProbe2);
            /*Printer::homeAxis(home_all_axis || com->hasX(),home_all_axis || com->hasY(),home_all_axis || com->hasZ());
              Printer::updateCurrentPosition(); */  
            Printer::moveTo(EEPROM::zProbeX3(),EEPROM::zProbeY3(),securityDistance,IGNORE_COORDINATE,EEPROM::zProbeXYSpeed());
            last = Printer::runZProbe(true,true);
            if(last<0) break;
            sum+= last;
            zProbe3 = last;
            ///Com::printFLN(Com::tZProbe,zProbe3);
            ///Com::printFLN(Com::tZProbe,Printer::zLength);
            Com::printFLN(Com::tZProbe,Printer::zLength - securityDistance + zProbe3);
            sum *= 0.33333333333333;
           
            //BED LEVELING IS OK
            if (zProbe1-zProbe3<levelTolerance && zProbe1-zProbe3>-levelTolerance && zProbe2-zProbe3<levelTolerance && zProbe2-zProbe3>-levelTolerance)
            {
            Com::printFLN(Com::tZSuccessful);  
            uint8_t home_all_axis = (com->hasNoXYZ());
            Printer::homeAxis(home_all_axis || com->hasX(),home_all_axis || com->hasY(),home_all_axis || com->hasZ());
            Printer::updateCurrentPosition();   
            Printer::moveTo(0.00,0.00,securityDistance,IGNORE_COORDINATE,EEPROM::zProbeXYSpeed());
            zPrinterHeight = Printer::runZProbe(true,true);
            Printer::homeAxis(home_all_axis || com->hasX(),home_all_axis || com->hasY(),home_all_axis || com->hasZ());
            Printer::updateCurrentPosition();  
            zMeasuredPrinterHeight = Printer::zLength - securityDistance + zPrinterHeight;
            Com::printFLN(Com::zPrinterHeight,zMeasuredPrinterHeight);
            if(com->hasS() && com->S == 2)
            {
               Printer::zLength = zMeasuredPrinterHeight;
               EEPROM::storeDataIntoEEPROM();
               Com::printFLN(Com::tZStored,Printer::zLength);
            }
            sum = 0,last,oldFeedrate = Printer::feedrate;
            zProbe1 = 0;
            zProbe2 = 0;
            zProbe3 = 0;
            zPrinterHeight = 0;
            zMeasuredPrinterHeight = 0;
            Printer::updateCurrentPosition();
            }
            else
            {
            Com::printFLN(Com::tZAdjust1,zProbe1-zProbe3);
            Com::printFLN(Com::tZAdjust2,zProbe2-zProbe3);
            sum = 0,last,oldFeedrate = Printer::feedrate;
            zProbe1 = 0;
            zProbe2 = 0;
            zProbe3 = 0;
...
//           printCurrentPosition();
    ...
    else if(com->hasM())    // Process M Code
    {
        ...
        case 42: //M42 -Change pin status via gcode
            ...
            break;
            case 198: //M198 - Heat SpiderBot enclosure
                    pinMode(FAN2_PIN, OUTPUT);
                    digitalWrite(FAN2_PIN, 255);
            break;
            case 199: //M199 - Stop Heat SpiderBot enclosure
                    pinMode(FAN2_PIN, OUTPUT);
